...
class Checkout extends PublicController
{
    public function run(): void
    {
        ...
        if ($this->isPostBack()) {
            $PayPalOrder = new \Utilities\Paypal\PayPalOrder(
                "test" . (time() - 10000000),
                "http://localhost:8080/KevsTrx/index.php?page=checkout_error",
                "http://localhost:8080/KevsTrx/index.php?page=checkout_accept"
            );
    ...
    public static function addPurchase($payment){
        ...
        purchase::insertPurchase($idFor,date("Y-m-d"),self::addToDb(),"Comprado en Kevstrx", $payment,\Dao\Security\Security::getCodigoByEmail($_SESSION['useremail']));
        if (isset($_SESSION['cart' . $xls])) {
            foreach ($_SESSION['cart' . $xls] as $product) {
                $usercod = $product['usercod'];
                $productId = $product['productid'];
